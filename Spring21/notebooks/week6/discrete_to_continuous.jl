...
# ╔═╡ baa6ace8-8b48-11eb-2bd0-316928a02a40
using Colors
...
In **individual-based** models, we literally model the behaviour of each individual. But often what we are actually interested in is a global picture of how the dynamics of a whole system, consisting of many individuals, evolves in time: how many infectious individuals there are in total at a given time, for example, or the behaviour of the whole stock market.

In this notebook we will see how we can start from an individual-based stochastic (probabilistic) model, and how sometimes we can find **deterministic** equations for a **macroscopic** (system-level) description.
...
md"""
This models many situations, for example: Time to failure of a light bulb or a piece of machinery; time for a radioactive nucleus to decay; time to recover from an infection, etc.
"""
...
# ╔═╡ bc634a66-8b48-11eb-0ab6-d992e090ee3e


# ╔═╡ bc4d1264-8b48-11eb-3c66-d339fc03baa6


# ╔═╡ e6337d66-8b48-11eb-1511-af35e56391ea
Colors.cgrad(:red, :white)

# ╔═╡ 60c513dc-8b49-11eb-303c-4b2e5095a44c


# ╔═╡ 7e1b61ac-08db-11eb-209e-1d6c328f5113
begin
	scatter(1:N, results[t],
		alpha=0.5, size=(400, 100), leg=false, ms=3,
		# c=Int.(results[t])
		c = [colorant"red", colorant"blue"]
	)
	
	for i in 1:N
		plot!([i, i], [0, 1], c=:gray, alpha=0.2)
	end
