...
# ╔═╡ 886b538e-e93b-11ea-1b28-21890cccb3c3
md"##"

# ╔═╡ 6a96049e-e936-11ea-14e8-ab66f3f5ead1
new_phil
...
# ╔═╡ da766da6-e93d-11ea-3749-937d1079e70a
md"## What does a blur do?

- Smooths out noise in the image

- By **averaging** a cell's value with its neighbours

- `new_image[i, j]` = $f$(neighbourhood of `[i, j]` in `image`)
"

# ╔═╡ 07ce2020-e93c-11ea-35c2-67b8698b6d50
md"## Phil is edgy

- Now let's detect Phil's edges"

# ╔═╡ 3cfa90ec-e93e-11ea-2d13-8520a8202608
md"# How does edge detection work?

- Think about a vertical edge in a noisy image

- How could we detect it?

$(br)

- The mathematical operation ends up being the same as a blur:
...
# ╔═╡ b1a5f8a6-e938-11ea-25b5-cf4e3000f11d
function convolve_image(image, kernel)
    ...
    new_image = similar(image)
    
    # (i, j) loop over the original image
    @inbounds for i in 1:size(image, 1)
        for j in 1:size(image, 2)
            
            accumulator = (0.0, 0.0, 0.0)
            
            # (k, l) loop over the pixels to include in the blur
            for k in -half_height:-half_height + height - 1
                for l in -half_height:-half_height + height - 1
                    ...
                    # clamp to boundary:
                    new_i < 1 && (new_i = 1)
                    new_i > size(image, 1) && (new_i = size(image, 1))
                    
                    new_j < 1 && (new_j = 1)
                    new_j > size(image, 2) && (new_j = size(image, 2))
                    
                    pixel = image[new_i, new_j]
                    accumulator = accumulator .+ kernel[k + half_height + 1, l + half_width + 1] .* (pixel.r, pixel.g, pixel.b)
                end
            end
        
            new_image[i, j] = RGBX{N0f8}(accumulator[1], accumulator[2], accumulator[3])
...
# ╔═╡ bfcd9ffe-e93a-11ea-3c1c-a939cb8ad40a
        ...
        for j in 1:size(M, 2)
            ...
            for k in -half_height:-half_height + height - 1
                for l in -half_width:-half_width + width - 1
                    ...
                    # clamp to boundary:
                    new_i < 1 && (new_i = 1)
                    new_i > size(M, 1) && (new_i = size(M, 1))
                    
                    new_j < 1 && (new_j = 1)
                    new_j > size(M, 2) && (new_j = size(M, 2))
                    
                    pixel = M[new_i, new_j]
                    accumulator += kernel[k + half_height + 1, l + half_width + 1] * pixel
                end
            end
...
# ╔═╡ b5f52f1c-e938-11ea-3739-15645f979f77
blur(n) = ones(n, n) ./ (n^2)
...
# ╔═╡ f3c71512-e938-11ea-2821-c3280ee9131a
convolve(poor_phil, blur(6))

# ╔═╡ 25f4e1e0-e939-11ea-1be0-df3cf32e3f12
expand(convolve(poor_phil, blur(blur_factor)))

# ╔═╡ dbf4418e-e939-11ea-3b54-c58f04dd6366
expand(edge_detect(convolve(poor_phil, blur(1))))
...
function camera_input(;max_size=200, default_url="https://i.imgur.com/VGPeJ6s.jpg")
"""
...
@bind raw_camera_data camera_input(;max_size=2000, default_url="https://i.imgur.com/VGPeJ6s.jpg")
...
# ╔═╡ b1b2f0d4-e93c-11ea-3443-712e098eecd9
convolve(grant, blur(15))
